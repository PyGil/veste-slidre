  ...
  return (
    <>
      <div className="container mx-auto pt-40 px-2">
        <h1 className="font-bold text-4xl mb-6 uppercase text-center">
          Kva skjer i{" "}
        ...
        // style={{
        //   backgroundImage:
        //     "radial-gradient(circle, rgb(28 138 255 / 20%), transparent 30%), radial-gradient(circle, rgb(152 73 248 / 20%), transparent 30%)",
        // }}
        >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
                {sortedData[month].map((event) => (
                  <Link href={`/events/${event.slug}`} key={event.slug}>
                    <Card className="pt-6 relative min-h-[420px] group bg-gradient-to-br from-blue-500 via-purple-500 to-red-500 bg-full overflow-hidden transition-all duration-200 hover:bg-lg">
                      ...
                      <Link
                        ...
                        className="z-30 absolute left-1/2 top-[70%] transform -translate-x-1/2 -translate-y-1/2 text-xl text-center opacity-0 group-hover:opacity-100 group-hover:top-1/2 transition-all duration-200"
                      >
                        <Button className="rounded-xl text-[16px]">Les mer om {event.title}</Button>
                      </Link>
                      {event.image && (
                        <>
                          <span className="opacity-0 group-hover:opacity-100 transition-all duration-200 absolute z-20 top-0 left-0 w-full h-full bg-gradient-to-br from-[#1c8aff]/90 via-transparent to-[#9849f8]/90" />
                          <Image
                            width={500}
                            height={350}
                            src={getImageData(event.image).url()}
                            alt={event.title}
                            className="rounded-lg object-cover absolute inset-0 w-full h-full transition-all duration-200 group-hover:scale-110 group-hover:blur-lg"
                          />
                        </>
                      )}
                    </Card>
